...
export function CreateCategoryModal({ categories, onClose, onCreated }: { categories: CategoryResponse[]; onClose: () => void; onCreated: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4" role="dialog" aria-modal="true">
    <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-lg space-y-4 rounded-lg bg-background p-6 shadow-lg">
      ...
      <div className="space-y-2"><Label htmlFor="category-title">Title</Label><Input id="category-title" {...register('title')} />{errors.title && <p className="text-sm text-destructive">{errors.title.message}</p>}</div>
      ...
      <div className="flex justify-end gap-2"><Button type="button" variant="secondary" onClick={onClose}>Cancel</Button><Button type="submit" disabled={isSubmitting}>{isSubmitting ? 'Creating...' : 'Create category'}</Button></div>
    </form>
  </div>;
}
